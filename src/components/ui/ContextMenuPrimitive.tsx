...
export const ContextMenuPrimitive = ({
    ...
    return (
        <div
            ...
            role="menu"
            tabIndex={-1}
            className="fixed z-50 min-w-44 py-1 bg-[var(--bg-secondary)] border border-[var(--border-subtle)] rounded shadow-lg outline-none text-[var(--text-primary)]"
            style={{
                top: position.top,
                left: position.left,
            }}
...
export const MenuItem = ({
    ...
    return (
        ...
        >
            {icon && <span className="mr-2.5 h-4 w-4 flex-shrink-0 flex items-center justify-center">{icon}</span>}
            <span className="flex-1 truncate">{label}</span>
            {shortcut && <span className="ml-4 text-xs opacity-60 font-mono tracking-widest uppercase">{shortcut}</span>}
        </div>
    );
};

export const MenuSeparator = () => (
    <div role="separator" className="h-px bg-[var(--border-subtle)] my-1" />
);
